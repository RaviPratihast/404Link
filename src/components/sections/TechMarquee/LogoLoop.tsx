...
export const LogoLoop = memo(
  ({
    ...
    const targetVelocity = useMemo(() => {
      ...
      const dirMultiplier = isVertical
        ? direction === "up"
          ? 1
          : -1
        : direction === "left"
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: string) => {
        ...
        const content = isNodeItem ? (
          ...
          // Marquee items use dynamic src/srcSet from data; next/image would require remotePatterns per host.
          // eslint-disable-next-line @next/next/no-img-element -- intentional for logo strip assets
          <img
            src={item.src}
            srcSet={item.srcSet}
            sizes={item.sizes}
